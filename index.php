          ...
          <div id="sticky">
            ...
            <ul class="mappeno" <?php
              ...
              <?php
                ...
                {
                  if (preg_match('/^(.*)_bonus(\d+)$/', $input, $matches)) {
                    $prefix = $matches[1];
                    $bonus = 'B' . $matches[2];
                    return $prefix . ' | ' . $bonus;
                  }
                ...
                if ($result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) {
                    $api_map = to_api_version($row['MapName']);
                    echo '<li class="selector" data-id="' . $row['MapName'] . '"';
                    echo " style=\"background-image: url('https://raw.githubusercontent.com/Letaryat/poor-sharptimermappics/main/pics/{$api_map}.jpg');";
                    echo " background-size: cover;";
                    echo '">';
                    echo convert_bonus_string($row['MapName']) . '</li>';
                  }
                }
              }
              ?>
        ...
        <div class="leaderboard">
          <div class="info">
            <div class="row">
              ...
              <span> <i class="fa-solid fa-arrows-rotate"></i> Completions </span>

            </div>
          </div>
          <div class="players">
            <?php
            populateMapLeaderboard($conn, $conn->real_escape_string($defaultmap));
            ?>
          </div>
        </div>
      </div>
